...
int16_t i16_ph2_current_filter = 0;             ////not used
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
  ...
  switch (ui8_hall_case) // 12 cases for each transition from one stage to the next. 6x forward, 6x reverse
  {
    ...
    case 64:
      ...
      i8_recent_rotor_direction = -i16_hall_order;         ////recent rotor direction = -1
...
void motor_autodetect() {
  ////to take input readings from motor
  enable_pwm();
...
void calculate_tic_limits(int8_t speed_limit) {
  ...
  // likely used to calculate the speed of a motor-controlled vehicle.
...
void motor_slow_loop(MotorStatePublic_t* motorStatePublic) {
  ...
  /*
  This loop likely runs at a lower frequency compared to the main control loop and handles tasks that don't require high-speed execution,
  such as updating certain motor parameters and performing safety checks.
  */
  ...
  if (q31_battery_voltage < p_MotorStatePublic->battery_voltage_min) {
    ////if battery low bhind certain threshold, send id and iq currents and zero to the motor....motor band
    MS.i_q_setpoint = 0;
    MS.i_d_setpoint = 0;
    MS.error_state = lowbattery;        ////error state will have error as low battery
    ...
    int16_t phase_current_limit = MSP->phase_current_limit / CAL_I;  ////yeh cal i kya h?
    if (MSP->i_q_setpoint_target > phase_current_limit) {
      MS.i_q_setpoint_temp = phase_current_limit;
    }
    ////yaha agar i_q_setpoint_target phase current k limit se jyada h, so it will cap it by sttein
    ////i_q_setpoint_temp to max phase current

    if (MSP->i_q_setpoint_target < -phase_current_limit) {
      MS.i_q_setpoint_temp = -phase_current_limit;
    }  ////same yaha but with negative current

    calculate_tic_limits(MSP->speed_limit);       ////yaha usse max and min speed limit pata chalegi
...
// injected ADC
/*
The injected mode is a mode where the ADC conversion can be "injected" during the conversion of regular 
channels due to some trigger (timer or something else). This is useful, for example in motor control application , 
to delay conversion until after some event is complete (such as transistor switching) so that the conversion noise is reduced.
*/
